...
func (r *TodoItemPostgres) Create(listId int, item todo.TodoItem) (int, error) {
	...
	createListItemsQuery := fmt.Sprintf("INSERT INTO %s (item_id, list_id) VALUES ($1, $2)",
		listsItemsTable)
	if _, err := tx.Exec(createListItemsQuery, itemId, listId); err != nil {
		tx.Rollback()
		return 0, nil
	}
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
	...
	query := fmt.Sprintf(`UPDATE %s ti SET %s FROM %s li, %s ul
                    WHERE li.item_id = ti.id AND ul.list_id = li.list_id AND ul.user_id = $%d AND ti.id = $%d`,
